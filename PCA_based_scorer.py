...
class AnomalyDD:
    def __init__(
        ...
        self.train_obs = train_obs.transpose(1, 0, 2).reshape(train_obs.shape[1], -1)
        self.val_obs = val_obs.transpose(1, 0, 2).reshape(val_obs.shape[1], -1)
        self.test_obs = test_obs.transpose(1, 0, 2).reshape(test_obs.shape[1], -1)

        self.train_forecast = train_forecast.transpose(1, 0, 2).reshape(
            train_forecast.shape[1], -1
        )
        self.val_forecast = val_forecast.transpose(1, 0, 2).reshape(
            val_forecast.shape[1], -1
        )
        self.test_forecast = test_forecast.transpose(1, 0, 2).reshape(
            test_forecast.shape[1], -1
        )
    ...
    def scorer(self, num_components):
        ...
        full_obs = np.concatenate(
            (self.train_obs, self.val_obs, self.test_obs), axis=0
        )
        full_forecast = np.concatenate(
            (self.train_forecast, self.val_forecast, self.test_forecast), axis=0
        )
        full_abs = np.absolute(full_obs - full_forecast)

        val_norm = error_normalizer(val_abs)
        test_norm = error_sw_normalizer(
            full_abs, self.window_length, self.batch_size, len(self.test_obs)
        )

        val_re, test_re, val_re_full, test_re_full = self.pca_model(
            val_norm, test_norm, num_components
        )
        ...
        print(
            "realtime_indicator shape:",
            realtime_indicator.shape,
            "anomaly_prediction shape:",
            anomaly_prediction.shape,
        )
...
def sliding_window(error_mat, window_size):
    return error_mat[
        ...
        + np.arange(error_mat.shape[0] - window_size)[:, None]
    ]
...
def error_sw_normalizer(error_mat, window_size, batch_size, test_size):
    ...
    for i in tqdm(range(num_batch)):
        ...
        sw_error_mat = sliding_window(
            error_mat[(start_idx - window_size) : end_idx, :], window_size
        )
